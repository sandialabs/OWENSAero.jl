...
const UNSTEADY_BASELINES = Dict(
    ("AC", true) => (
        Rp1 = 14.61798502718863,
        Tp1 = -1.3578451054187242,
        Rpmean = 5.478111754622732,
        Tpmean = 6.599005743637965,
        Vloc1 = 42.65570346549178,
        Re1 = 445031.47870641906,
    ),
    ("AC", false) => (
        Rp1 = 12.529927964477698,
        Tp1 = -1.408546292257496,
        Rpmean = 4.660093508900539,
        Tpmean = 4.321887489076866,
        Vloc1 = 42.66169039146707,
        Re1 = 445093.9409401469,
    ),
    ("DMS", true) => (
        Rp1 = 17.8724152206306,
        Tp1 = -1.2526644527903148,
        Rpmean = 6.733379018517041,
        Tpmean = 8.114827894681484,
        Vloc1 = 42.5286710408061,
        Re1 = 443706.1359454618,
    ),
    ("DMS", false) => (
        Rp1 = 17.8724152206306,
        Tp1 = -1.2526644527903148,
        Rpmean = 18.419804953792056,
        Tpmean = 4.136206532176461,
        Vloc1 = 42.5286710408061,
        Re1 = 443706.1359454618,
    ),
)
...
const IFW_UNSTEADY_BASELINES = Dict(
    "AC" => (
        Rp1 = -2.6331031705280084,
        Tp1 = -1.2559507588560983,
        Rpmean = 2.3980398649584904,
        Tpmean = 0.30031142795009114,
        Vloc1 = 38.56407465412821,
        Re1 = 402343.0813221861,
    ),
    "DMS" => (
        Rp1 = -3.958783356071784,
        Tp1 = -1.272566646170957,
        Rpmean = 4.65480460237821,
        Tpmean = -0.017477189310114483,
        Vloc1 = 38.95222326170589,
        Re1 = 406392.6769156071,
    ),
)
...
function aerowrapper(
    ...
    if steady
        CP, Th, Q, Rp, Tp, Zp, Vloc, CD, CT, a, awstar, alpha, cl, cd_af, thetavec, Re =
            OWENSAero.steady(turbine, env)
    else
        N_Rev = 2
        tau = [0.3, 3.0]
        ...
        for step1 = 1:(ntheta*N_Rev)
            ...
            cd_temp,
            thetavec_temp,
            Re_temp = OWENSAero.Unsteady_Step(turbine, env, us_param, step1)
...
for AeroModel in ["AC", "DMS"]
    ...
    @testset "TEST Unsteady Method with ifw $AeroModel" begin
        ...
        CP, Th, Q, Rp, Tp, Zp, Vloc, CD, CT, a, awstar, alpha, cl_af, cd_af, thetavec, Re =
            aerowrapper(xin; returnall = true, AeroModel, steady = false, ifw = true)
        test_unsteady_outputs(
            AeroModel,
            true,
            Rp,
            Tp,
            ...
            Vloc,
            Re,
            IFW_UNSTEADY_BASELINES[AeroModel],
        )
        # PyPlot.figure()
    ...
    @testset "TEST Unsteady Method $AeroModel" begin
        ...
        for RPI in [true, false]
            ...
            cd_af,
            thetavec,
            Re = aerowrapper(xin; RPI, returnall = true, AeroModel, steady = false)
            test_unsteady_outputs(
                AeroModel,
                RPI,
                Rp_us,
                ...
                Vloc,
                Re,
                UNSTEADY_BASELINES[(AeroModel, RPI)],
            )
    ...
    @testset "TEST Wind Direction $AeroModel" begin
        ...
        if AeroModel=="DMS"
            ...
            CP_ref, Rp_ref, Tp_ref, Vloc_ref, alpha_ref, thetavec_ref =
    ...
    @testset "TEST Accuracy $AeroModel" begin
        ...
        CP, Th, Q, Rp, Tp, Zp, Vloc, CD, CT, a, awstar, alpha, cl_af, cd_af, thetavec, Re =
            aerowrapper(xin; returnall = true, AeroModel)
        ...
        expected_CP = AeroModel == "DMS" ? 0.4462951778917397 : CP_old
        ...
        @test isfinite(CP)
        @test CP ≈ expected_CP rtol=fixture_rtol atol=1e-12
        ...
        for ii = 1:length(CP)
            expected_CP_ii = AeroModel == "DMS" ? expected_CP : expected_CP[ii][1]
            @test isapprox(CP[ii], expected_CP_ii; atol = tol)
        end
        # PyPlot.figure()
