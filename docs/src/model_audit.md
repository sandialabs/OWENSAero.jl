...
| HAWT CCBlade rigid BEM adapter | Exported helper path for CCBlade section construction, operating points, and a rigid-rotor solve. It is not yet coupled into OWENS structural dynamics. | API tests pin CCBlade section/operating-point conventions, force/power coefficients, per-station induction/load arrays, invalid inputs, and operating-point ForwardDiff behavior. | Add a documented rigid-rotor HAWT example, OpenFAST/AeroDyn BEM validation metrics, and structural load mapping with global rotor/tower frame bookkeeping. |
