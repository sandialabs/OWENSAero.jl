...
| HAWT AeroDyn input readers | Exported helpers parse AeroDyn primary input files, v15 blade-definition files, and AirfoilInfo v1 polar tables into typed named tuples, OWENSAero-compatible airfoil callables, and a CCBlade solve bridge. | API tests pin BEM option flags, file lists, input-column mapping, declared blade-node handling, ignored extra fixture rows, geometry units, AirfoilInfo metadata, `Cm` propagation, root-station policies, synthetic CCBlade solve metrics, missing-`Cm` defaults, and invalid input failures. | Use the solve bridge in a short OpenFAST/AeroDyn BEM comparison after wind-source, drag-in-induction, and torque/power sign conventions are normalized. |
