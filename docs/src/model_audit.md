...
| HAWT AeroDyn input readers | Exported helpers parse AeroDyn v15 blade-definition files and AirfoilInfo v1 polar tables into typed named tuples and OWENSAero-compatible airfoil callables. | API tests pin declared blade-node handling, ignored extra fixture rows, geometry units, AirfoilInfo metadata, `Cm` propagation, missing-`Cm` defaults, and invalid input failures. | Use the readers in a short OpenFAST/AeroDyn BEM comparison after root-station, wind-source, and torque/power sign conventions are normalized. |
