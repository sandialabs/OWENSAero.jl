...
| `Q` | torque | N m |
| `CP`, `CD`, `CT` | nondimensional performance and force coefficients | dimensionless |
...
Airfoil reader functions expect angle-of-attack and coefficient tables compatible with the selected model. The dynamic-stall path carries additional state for prior angle of attack and Boeing-Vertol flags. Because these state variables affect transient loads, unsteady tests should pin the full output vector for a named input file and time history.
