...
function _validate_hawt_station_inputs(radial_positions, chord, twist)
    r = _validated_hawt_station_vector(radial_positions, "radial_positions"; positive = true)
...
function _hawt_airfoil_vector(airfoils, nstations)
    if airfoils isa AbstractVector
        ...
        length(airfoils) == nstations ||
            throw(ArgumentError("airfoils must be a single callable or one callable per station"))
...
function _validate_hawt_operating_values(
    ...
    rotor_speed == zero(rotor_speed) &&
        throw(ArgumentError("rotor_speed must be nonzero"))
...
function _validate_hawt_rotor_geometry(radial_positions, hub_radius, tip_radius)
    ...
    hub_radius <= first(radial_positions) ||
        throw(ArgumentError("hub_radius must be less than or equal to the first radial station"))
    tip_radius >= last(radial_positions) ||
        throw(ArgumentError("tip_radius must be greater than or equal to the last radial station"))
    return nothing
end
...
function ccbladeHAWTSolve(
    ...
    npts isa Integer && npts > 0 ||
        throw(ArgumentError("npts must be a positive integer"))
    ...
    rotor = CCBlade.Rotor(hub_radius, tip_radius, num_blades; precone, turbine = true)
