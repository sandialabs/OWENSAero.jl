...
twist = 0.0
delta = 0.0
af = VAWTAero.readaerodyn("$path/airfoils/NACA_0012_mod.dat")
B = 3
solidity = 0.25
chord = solidity*r/B
...
@testset "VAWTAero.jl one turbine" begin
    ...
    for i in eachindex(tsrvec)
        ...
        CP,_,_, Rp, Tp, Zp,_,_,CT,_,_,_,_,_,theta,_ = VAWTAero.AC_steady(turbines, env)
        CPvec[i] = CP[1]
        CTvec[i] = CT[1]
        Rpvec[i] = Rp[1]
        Tpvec[i] = Tp[1]
...
println("two/multiple turbine case needs to be conformed to the VAWTAero io before it is used")
# @testset "VAWTAero.jl two turbines" begin
#     # -----------  two turbines -----------------
#     tsr = 3.5
#     omega = Vinf*tsr/r
#
#     turbines = Array{VAWTAero.Turbine}(undef,2)
#     turbines[1] = VAWTAero.Turbine(r, chord, twist, delta, B, af, omega, 0.0, 0.0)
#     turbines[2] = VAWTAero.Turbine(r, chord, twist, delta, B, af, -omega, 0.0, 2*r)
#     CT, CP, Rp, Tp, Zp, theta = VAWTAero.actuatorcylinder(turbines, env, ntheta)
#
#     #write to file
