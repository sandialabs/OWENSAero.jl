...
function AC(turbines, env; w=zeros(Real,2*turbines[1].ntheta), idx_RPI=1:2*turbines[1].ntheta,solve=true,ifw=false)
    ...
    return CP, q ,Q, Rp, Tp, Zp, sqrt.(Vn.^2 .+ Vt.^2), CT, CT, a, w, alpha, cl, cd, theta, Re
...
#
#     if nturbines == 1
#              # CP,      Th,   Q,      Rp, Tp, Zp, Vloc,                  CD,      CT,astar,                     _,                       alpha, cl, cd_af, thetavec, Re = VAWTAero.steady(turbine, env)
