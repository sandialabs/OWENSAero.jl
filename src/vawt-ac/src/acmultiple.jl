...
function radialforce(uvec, vvec, thetavec, turbine, env)
    ...
    qdyn = 0.5*rho*W.^2
    Rp = cn.*qdyn*chord - F_addedmass_Np  # TODO CM: correct?
    Tp = (rotation*ct.*qdyn.*chord + F_addedmass_Tp)./cos.(delta)  # TODO CM: correct?
    Zp = (cn.*qdyn.*chord - F_addedmass_Np).*tan.(delta)  # TODO CM: correct?
